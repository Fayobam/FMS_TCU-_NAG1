...
| **V23** | **Pre-first-drive shakedown pass (full multi-agent code review).** Five targeted, low-complexity hardening changes — no behaviour change to the validated shift path: (1) **Engagement now latches off the 4-bit shifter plate alone** (`isForwardRange()`), decoupled from the multiplexed pin-39 P/N switch — an open/cold ATF temp sensor (shared line, reads >3.0 V) can no longer silently block drive engagement. (2) **Money-shift guard is output-sensor-independent**: predicts post-downshift speed from BOTH output×ratio AND turbine×(ratio_t/ratio_c) and trusts the higher, so a dead output sensor reading 0 can't defeat the over-rev block. (3) **Reverse (R1/R2) reuses 2nd-gear holding pressure** (deterministic line; no separate reverse cal/adapt). (4) **`ENABLE_RP_LOCK` default OFF** — lock hardware not fitted; verify polarity on the bench before enabling. (5) **MCPWM capture init fails soft** instead of `ESP_ERROR_CHECK`→boot-loop: on any allocation error speed sensing is disabled (speeds read 0, shifts run on time backstops) and the TCU still boots. **Left as-is by decision (good enough / firm-but-adapts):** TCC 1000 Hz carrier, firm fill/holding defaults, `cl_spc_enable=1`/Kp=80, and the pulse-then-latch shift-valve model (declared a non-issue). No NVS/struct change → existing cal preserved. Flash 86.0%. |
| **V22** | **Variant-aware clutch defaults + clutch-speed transitions (UN52 backport Phase 0/1b finish).** (1) Clutch pressure-model defaults are now **physics-seeded per gearbox** (`seedClutchModelForVariant`): W5A330 small NAG (~4000/coef ≈ 27 mBar/Nm, 16 bar line) and W5A580 big NAG (×~1.76 friction, 18 bar) — switching the Transmission dropdown reloads that box's defaults live. (2) **Phase 1b**: `cl_speed_transitions` (NVS, default OFF) makes FILL→TORQUE exit on off-going clutch slip > `CLUTCH_MOVE_RPM` and INERTIA/CATCH sync on on-coming slip ≤ `CLUTCH_SYNC_RPM`, with the ratio path as backstop — enable after bench-verifying the clutch-speed signs. EP_MAGIC→'NAG9' (re-seeds NVS). No clean OEM friction table exists in any UN52 version (old = tunable working-pressure map, new = per-car EGS blob), so seeds are physics-based by design. Flash 86.0%. |
| **V21** | **Phase-aligned torque-cut (UN52 backport Phase 5).** The rusEFI shift-retard GPIO is now a phase-aligned **window** instead of an INERTIA-only pulse: lead-in during TORQUE (retard latency → request before the speed change), hold through INERTIA, release at sync. High-load power-upshift only (`ENABLE_TORQUE_CUT`/`TORQUE_CUT_MIN_LOAD`, default off). Exposed as `tqCut` telemetry, Shift-CSV flag bit3, and a ✂ marker on the dashboard class badge. Single GPIO = window control only (amplitude/ramp would need CAN). No NVS change. Flash 86.0%. |
